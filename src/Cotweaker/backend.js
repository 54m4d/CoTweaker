...
function makeActive(button) {
    const buttons = document.querySelectorAll('.quick-controls .button');

    buttons.forEach(btn => btn.classList.remove('active'));
    button.classList.add('active');

    if (button.id === "removeBloatwareButton") {
        document.getElementById("appxHeader").classList.add("glassy-box");
        var appxHeader = document.getElementById('appxHeader');
        appxHeader.style.display = 'block';
        appxHeader.scrollIntoView();
    } else if (button.id === "checkPrivacyButton") {
        document.getElementById("dashboardHeader").classList.add("glassy-box");
        var systemHeader = document.getElementById('dashboardHeader');
        systemHeader.style.display = 'block';
        systemHeader.scrollIntoView();
    } else if (button.id === "checkCopilotStatusButton") {
        console.log("Searching AI features...");
        window.chrome.webview.postMessage('detectAI');
    } else {
        // handling other cases...
    }
}

// Handle scrolling to specific elements, here appxHeader > Pinned apps > All apps button
function scrollToAppxHeader() {
    var appxHeader = document.getElementById('appxHeader');
       appxHeader.style.display = 'block';	// Get element with ID "appxHeader"
    if (appxHeader) { 
        appxHeader.scrollIntoView({ behavior: "smooth" }); // Scroll to element
    }
}
...
function toggleSwitch() {
     ...
     window.chrome.webview.postMessage('toggleCopilot');
}
...
// Static Check for updates button
...
/* 
Interop code > Checkboxes and Buttons for Privacy Dashboard
*/
...
// Remove the three apps in Pinned apps section
